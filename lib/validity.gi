...
function( Q, rho, input_str )
    ...
    sigma_eps := function( Q, rho1 )
        local Composable_Matrix, IsComposable, Incoming_Arrows, Outgoing_Arrows,
        ...
        Incoming_Arrows := function(Q,v)
            ...
            a := [];
            for i in [1..NumberOfArrows(Q)] do
                Append(a, [SIntChar(String(TargetOfPath(
                ArrowsOfQuiver(Q)[i]))[2]) - SIntChar('0')]);
            od;
            b := [];
        ...
        Outgoing_Arrows := function(Q,v)
            ...
            a := [];
            for i in [1..NumberOfArrows(Q)] do
                Append(a, [SIntChar(String(SourceOfPath(
                ArrowsOfQuiver(Q)[i]))[2]) - SIntChar('0')]);
            od;
            b := [];
            for i in [1..Length(a)] do
        ...
        temp_in := function(Q,i,j)
            return SIntChar(String(Incoming_Arrows(Q,i)[j])[1])
            - SIntChar('a') + 1;
        end;

        temp_out := function(Q,i,j)
            return SIntChar(String(Outgoing_Arrows(Q,i)[j])[1])
            - SIntChar('a') + 1;
        end;
        ...
        for i in [1..NumberOfVertices(Q)] do
            if Length(Incoming_Arrows(Q, i)) = 2 then
                ...
                if Length(Outgoing_Arrows(Q, i)) = 1 then
                    ...
                    if IsComposable(Q, rho, Outgoing_Arrows(Q,i)[1],
                      Incoming_Arrows(Q, i)[1]) = 1 or IsComposable(Q, rho,
                      Outgoing_Arrows(Q, i)[2],Incoming_Arrows(Q,i)[2]) = 1 then
                        sigma[temp_out(Q, i, 1)] := -1;
...
[ IsQuiver, IsList, IsString ], 0,
function( Q, rho, input_str )
    local sigma_eps, arr, temp, temp1, La, Lb, Ra, Rb, direct_left, direct_right,
    inverse_left, inverse_right, Incoming_Arrows, Outgoing_Arrows, array, sigma,
    eps, k, kQ, rel, arr1, gens, i, j;
    ...
    direct_right := function(Q, rho, input_str, sigma, eps)
        ...
        if SIntChar(temp[Length(temp)]) > 96 then
            ...
            - SIntChar('0');
            if Length(Incoming_Arrows(Q, x)) = 2 then
                if SIntChar(String(Incoming_Arrows(Q, x)[1])[1])
