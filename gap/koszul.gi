#############################################################################
# Param  :  a Quiver object q
# Return : a list of all paths of length two in q, sorted by <
#
# Fails with error message if q is not a Quiver object.
#
# This could be made more efficient for large quivers with few
...
#############################################################################
# Param gens : a list of generators of an ideal I of some path algebra
# Return     : true, if I is a quadratic ideal; false otherwise
#
...
#############################################################################
# Param gens : a list of generators of an ideal I of some path algebra
# Return     : if all is well, kQ_op/Iperp (the Koszul dual);
#              otherwise, fail with error message (to STDERR)
#
# The idea:
#
...
#I := Ideal(pa_op,basisOfIperp);
#  gb := GBNPGroebnerBasis(basisOfIperp,pa_op);
#  GroebnerBasis(I,gb);
 
#  return pa_op/Ideal(pa_op,basisOfIperp);
#  return pa_op/I;
