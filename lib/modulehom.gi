...
#######################################################################
##
#O  ADRAlgebraOfAlgebra( <A> )
##
##  Given a finite dimensional quiver algebra <A> this function 
##  constructs the endomorphism algebra of the <A>-modules
##     <\oplus_{i=1}^n A/\rad(A)^i,
##  where n = Loewy length of <A>, which is known to have finite
##  global dimension <= n + 1.
##  
InstallMethod( ADRAlgebraOfAlgebra, 
  "for a quiveralgebra",
  true,
  [ IsQuiverAlgebra ], 0,
  function( A )
    
  local k, N, modules, inclusions, newmodules, newinclusions, i, temp, j, B;
  ...
  N := LoewyLength( A );
  modules := IndecProjectiveModules( A );
  inclusions := List( modules, t -> IdentityMapping( t ) );
  newmodules := [];
  for i in [ 1..N ] do
    newinclusions := [];
    temp := List( inclusions, t -> RadicalOfModuleInclusion( Source( t ) ) );
    for j in [ 1..Length( temp ) ] do
      if Dimension( Source( temp[ j ] ) ) > 0 then
        Add( newmodules, CoKernel( temp[ j ] * inclusions[ j ] ) );
        Add( newinclusions, temp[ j ] * inclusions[ j ] );
      fi;
    od;
    inclusions := newinclusions;
  od;
  modules := Concatenation( modules, Reversed( newmodules ) );
  B := EndOfBasicModuleAsQuiverAlgebra( modules );
  SetIsFiniteGlobalDimensionAlgebra( B, true );
...
#######################################################################
##
#O  ADRAlgebraOfBasicModule( <L> )
##
##  Given a list <L> of non-isomorphic modules over a finite 
##  dimensional quiver algebra, this function constructs the 
##  endomorphism algebra of N, where N generate the same additive
##  subcategory as the modules
##    \oplus_{i=1}^n M/\rad^i( M ),
##  where M is the direct sum of all modules on the list <L> and 
##  n = Loewy length of <M>. Must be over a finite field.
##  
InstallMethod( ADRAlgebraOfBasicModule, 
  "for a PathAlgebraMatModule",
  true,
  [ IsList ], 0,
  function( L )
    
  local A, k, N, modules, inclusions, newmodules, newinclusions, i, temp, j, B;
      
  if Length( L ) = 0 then 
    Error( "The entered list of modules is empty.\n" );
  fi;
  A := RightActingAlgebra( L[ 1 ] );
  if not ForAll( L, l -> RightActingAlgebra( l ) = A ) then 
      Error( "Not all modules are over the same algebra.\n" );
  fi;
  k := LeftActingDomain( L[ 1 ] );
  if not IsFinite( k ) then 
    Error( "The entered module is not over an algebra over a finite field.\n" );
  fi;
  if not IsFiniteDimensional( A ) then 
    Error( "The entered algebra is not a finite dimensional algebra.\n" );
  fi;
  N := Maximum( List( L, l -> LoewyLength( l ) ) );
  inclusions := List( L, t -> IdentityMapping( t ) );
  newmodules := [];
  for i in [ 1..N ] do
    newinclusions := [];
    temp := List( inclusions, t -> RadicalOfModuleInclusion( Source( t ) ) );
    ...
    inclusions := newinclusions;
  od;
  newmodules := Concatenation( L, Reversed( newmodules ) );
  newmodules := MakeBasicListOfModules( newmodules );  
  B := EndOfBasicModuleAsQuiverAlgebra( newmodules );
...
#######################################################################
##
#O  MakeBasicListOfModules( <L> )
##
##  Given a list <L> of modules over a finite dimensional quiver
##  algebra, this function constructs a new list <M> of modules such 
##  that the additive subcategory generated by L and by M are the same
##  and the direct sum of the modules in M is a basic module. Must be 
##  over a finite field. 
##  
InstallMethod( MakeBasicListOfModules, 
  "for a PathAlgebraMatModule",
  true,
  [ IsList ], 0,
  function( L )
    
  local A, k, n, newmodules, i, j, temp;
  ...
  n := Length( L );
  n := Length( L );
  newmodules := ShallowCopy( L );
  for i in [ 1..n - 1 ] do
    if newmodules[ i ] = 0 then 
      break;
    fi;
    j := i + 1;
    while j in [ i + 1..n ] do
      if newmodules[ j ] <> 0 then
        temp := MaximalCommonDirectSummand( newmodules[ i ], newmodules[ j ] );
        if temp <> false then
          if Dimension( temp[ 3 ] ) > 0 then 
            newmodules[ j ] := temp[ 3 ];
          else
            newmodules[ j ] := 0;   
          fi;
    ...
    od;
  od;
  newmodules := Filtered( newmodules, m -> m <> 0 );
  newmodules := Flat( List( newmodules, DecomposeModule ) );
